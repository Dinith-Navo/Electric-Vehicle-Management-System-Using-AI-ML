...
from ..recommendation.recommendation.service import RecommendationScoringEngine, calculate_haversine_distance
from database import charging_stations_collection
import logging

logger = logging.getLogger(__name__)

# Realistic default charging stations used when MongoDB has not been seeded yet
DEFAULT_STATIONS = [
    {
...
class ChargingStationService:
    ...
    async def get_all_stations(filters: Optional[StationFilterParams] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Failed to query MongoDB for charging stations: {e}")

        # Fallback to rich default stations if DB is empty
        station_pool = db_stations if db_stations else [dict(s) for s in DEFAULT_STATIONS]

        # 2. Score and rank stations
